...
GCS_STAGING_PREFIX = airflow_var(
    ...
    "tmp/fxreplay_prod_mysql_to_bigquery_30min/users_snapshots",
)
...
def recreate_table(
    ...
    logger.warning("Recreating BigQuery table %s: %s", table_ref, reason)
    client.delete_table(table_ref, not_found_ok=True)
    table = bigquery.Table(table_ref, schema=desired_schema)
    return client.create_table(table, exists_ok=True)
...
def reconcile_table_schema(
    ...
    table_ref: str,
    desired_schema: list[bigquery.SchemaField],
    allow_recreate_on_incompatible_type_change: bool = False,
) -> bigquery.Table:
    ...
    for field_name, current_field in current_map.items():
        ...
        if current_field.field_type.upper() != desired_field.field_type.upper():
            ...
            try:
                ...
                message = str(exc)
                if allow_recreate_on_incompatible_type_change:
                    return recreate_table(
                        ...
                        (
                            ...
                            f"from {current_field.field_type} to {desired_field.field_type}: "
                            f"{message}"
                        ),
                    )
                if (
                    ...
                    and "Quota exceeded" not in message
                ):
                    raise
                logger.warning(
...
def sync_table_schemas(table_config: dict[str, Any]) -> dict[str, Any]:
    ...
    try:
        ...
        raw_table = reconcile_table_schema(
            ...
            allow_recreate_on_incompatible_type_change=True,
        )
...
def sync_table_raw(
    ...
    try:
        ...
        raw_bool_columns = {
            ...
            if str(field.field_type).upper() == "BOOL"
        }
        source_incremental_column = table_config["merge_config"]["source_incremental_column"]
        is_full_snapshot = source_incremental_column is None
        ...
        snapshot_gcs_uri: str | None = None
        snapshot_file = None
        use_snapshot_file = is_full_snapshot and str(table_config["task_name"]) == "users"
        if use_snapshot_file:
            ...
            logger.info(
                "Users snapshot staging target for %s: gs://%s/%s",
                raw_table_ref,
                GCS_STAGING_BUCKET,
                gcs_blob_name,
            )
            ...
            while True:
                ...
                if is_full_snapshot:
                    if use_snapshot_file:
                        ...
                        snapshot_rows.extend(json_rows)
                else:
        ...
        if is_full_snapshot:
            try:
                if use_snapshot_file and snapshot_file_path is not None:
                    if total_rows > 0:
                        snapshot_gcs_uri = upload_file_to_gcs(
                            snapshot_file_path,
                            gcs_blob_name,
                        )
                        logger.info(
                            "Uploaded users snapshot for %s to GCS: %s",
                            raw_table_ref,
                            snapshot_gcs_uri,
                        )
                ...
                if snapshot_file_path and os.path.exists(snapshot_file_path):
        ...
        return {
            ...
            "snapshot_gcs_uri": snapshot_gcs_uri,
            "last_timestamp": (
