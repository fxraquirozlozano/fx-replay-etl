...
def delete_final_window(
    ...
    if target_incremental_column is None:
        raise ValueError(
            f"Backfill delete requires target_incremental_column for {final_table_ref}."
        )

    filters: list[str] = []
    if isinstance(start_timestamp, datetime):
        filters.append(f"`{target_incremental_column}` >= TIMESTAMP('{start_timestamp.isoformat()}')")
    if isinstance(end_timestamp, datetime):
        filters.append(f"`{target_incremental_column}` < TIMESTAMP('{end_timestamp.isoformat()}')")
    if not filters:
        raise ValueError(f"Backfill delete window is required for {final_table_ref}.")
...
with DAG(
    dag_id="fxr_postgres_to_bigquery_ops_backfill",
    description=(
        "Backfill manual para tablas operativas PostgreSQL de FXR hacia BigQuery "
        "raw y final. Soporta dag_run.conf con start_timestamp y end_timestamp."
    ),
    ...
    tags=["postgres", "bigquery", "fxr", "raw", "backfill"],
) as dag:
